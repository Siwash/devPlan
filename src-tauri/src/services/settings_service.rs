...
use crate::models::settings::{ExcelTemplateConfig, LlmConfig};
use rusqlite::Connection;
use std::collections::HashMap;

pub fn get_llm_config(conn: &Connection) -> Result<Option<LlmConfig>, String> {
    let settings =
        settings_repo::get_settings_by_category(conn, "llm").map_err(|e| e.to_string())?;
...
pub fn get_excel_template_config(conn: &Connection) -> Result<Option<ExcelTemplateConfig>, String> {
    ...
    let column_mapping = match settings.get("excel_template.column_mapping") {
        Some(s) if !s.is_empty() => {
            serde_json::from_str(s).map_err(|e| format!("Failed to parse column_mapping: {}", e))?
        }
        _ => Vec::new(),
    };

    let header_row = settings
        .get("excel_template.header_row")
        .and_then(|v| v.parse::<i32>().ok());
    let default_sheet_name = settings.get("excel_template.default_sheet_name").cloned();
    let export_columns = match settings.get("excel_template.export_columns") {
        Some(s) if !s.is_empty() => Some(
            serde_json::from_str::<Vec<String>>(s)
                .map_err(|e| format!("Failed to parse export_columns: {}", e))?,
        ),
        _ => None,
    };
    ...
    if column_mapping.is_empty()
        && header_row.is_none()
        && default_sheet_name.is_none()
        && export_columns.is_none()
    {
        return Ok(None);
    }

    Ok(Some(ExcelTemplateConfig {
        column_mapping,
        header_row,
        default_sheet_name,
        export_columns,
...
pub fn save_excel_template_config(
    conn: &Connection,
    ...
    if let Some(ref sheet_name) = config.default_sheet_name {
        settings_repo::set_setting(
            conn,
            "excel_template.default_sheet_name",
            sheet_name,
            "excel_template",
        )
        .map_err(|e| e.to_string())?;
    }
    if let Some(ref export_columns) = config.export_columns {
        let export_columns_str = serde_json::to_string(export_columns)
            .map_err(|e| format!("Failed to serialize export_columns: {}", e))?;
        settings_repo::set_setting(
            conn,
            "excel_template.export_columns",
            &export_columns_str,
            "excel_template",
        )
        .map_err(|e| e.to_string())?;
