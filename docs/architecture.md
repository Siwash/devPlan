...
│              React 前端 (Vite)               │
│  ┌──────────┐ ┌──────────┐ ┌──────────┐ ┌──────────┐   │
│  │ Calendar  │ │  Tasks   │ │ Members  │ │ Standup  │   │
│  │  Views    │ │  CRUD    │ │ Manage   │ │  Module  │   │
│  └────┬─────┘ └────┬─────┘ └────┬─────┘ └────┬─────┘   │
│       │             │            │            │          │
│  ┌────┴─────────────┴────────────┴────────────┴────┐    │
│  │         Zustand Store               │    │
...
|------|------|------|
| id | INTEGER PK | 自增ID |
| meeting_id | INTEGER FK | 关联早会 |
| developer_id | INTEGER FK | 开发人员 |
| done_items | TEXT(JSON) | 昨日完成 |
| plan_items | TEXT(JSON) | 今日计划 |
| blockers | TEXT(JSON) | 问题/阻碍 |

### standup_task_links 表
| 字段 | 类型 | 说明 |
|------|------|------|
| id | INTEGER PK | 自增ID |
| entry_id | INTEGER FK | 关联条目 |
| task_id | INTEGER FK | 关联任务 |
| link_type | TEXT | 关联类型(done/plan/blocker) |
