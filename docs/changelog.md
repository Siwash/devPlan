...
- **迭代管理 CRUD**：新增迭代管理页面，支持新增/编辑/删除迭代；删除时有关联任务弹窗警告并解关联（事务保护），无关联任务直接删除
- **任务列表日期范围查询**：TaskFilter 新增开始/结束日期筛选，支持单侧/双侧日期交叉命中查询；日期 start>end 时禁用查询按钮
- **导出支持选中行模式**：ExportDialog 增加”按过滤条件/按选中行”互斥切换，切换时自动重置对方参数；选中行模式空选时禁用导出按钮；filter 模式日期无效时禁用导出按钮
- **Sprint 列表展示关联任务数**：Sprint 查询增加子查询统计关联任务数，前端表格新增”关联任务”列
- **创建迭代名称查重**：create_sprint 新增名称唯一性校验
- **编辑迭代支持清空字段**：update_sprint 采用空字符串标记清空，后端转 NULL

### 早会模块优化（来自 Unreleased）

- **早会模块升级为高效办公工作台**：围绕”Calm Productivity / 办公控制台”方向重构早会页面视觉层级，让今日编辑区成为主中心，历史复用区收敛为低噪音辅助面板。
- **编辑器与历史复用区视觉统一**：统一 Markdown 编辑器壳层、工具栏、预览区、拖拽投放态、历史日期侧栏和历史引用块的样式，减少拼装感与装饰性噪音。
- **历史拖拽与引用体验强化**：在保留原有 Markdown 引用与插入逻辑的前提下，让历史段落的点击/拖拽复用更清晰、更符合桌面办公操作习惯。
- **早会页容器与密度进一步收敛**：针对页签壳层不协调、内容容器挤压、顶部头部过高的问题，压缩早会页头与外围留白，让 standup 页面更像紧凑工作区而非被多层卡片包裹的页面。
- **历史复用改为可折叠紧凑侧栏**：历史区支持收起/展开，并将日期浏览精简为更高密度的列表式交互，减少对主编辑区的占用。
...
- 复测早会 Playwright 场景：
  ...
  - 今日待办显式导入为早会模板
  - 空历史状态展示
- 复测 `standupMarkdownUtils` 工具函数与前端构建，确认视觉升级未破坏现有 Markdown/拖拽/待办导入能力。
...
- **早会编辑器升级为开源 Markdown Editor**：引入 `@uiw/react-md-editor`，提供工具栏增强编辑、实时预览模式切换与异常回退到基础文本编辑能力
- **历史记录改为滚轮式回顾**：历史日期从下拉切换升级为滚轮/卡片式连续浏览，支持更顺手地回看最近 180 天早会内容
- **历史引用与待办导入增强**：拖拽历史段落时会携带来源日期并插入为 Markdown 引用，同时支持将当天待办一键导入为可补充进度/风险/备注的早会模板
